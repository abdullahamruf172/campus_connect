	...
	if(isset($_POST['sub'])){
		...
		if(strlen($content) > 250){
			...
			if(strlen($upload_image) >= 1 && strlen($content) >= 1){
				...
				$insert = "insert into posts (id, post_content, upload_image, post_date) values('$id', '$content', '$upload_image.$random_number', NOW())";
				...
				if($run){
					...
					$update = "update users set posts='yes' id='$id'";
				...
				<div id='posts' class='col-sm-6'>
					<div class='row'>
						...
						<div class='col-sm-6'>
							...
							<h4><small style='color:#FFFFFF;'>Updated a post on <strong>$post_date</strong></small></h4>
						</div>
						<div class='col-sm-4'>
						</div>
					</div>
					...
					<a href='single.php?post_id=$post_id' style='float:right;'><button class='btn btn-success'>Comment</button></a><br>
				</div>
				<div class='col-sm-3'>
				</div>
			</div><br><br>
...
if($p_id!=$post_id){
	...
	if($comment==""){
	   ...
	   // echo" <script>window.open('single_post.php?post_id=$post_id','_self')</script>";
		...
		$insert = "insert into comments (post_id,id,comment,comment_author,date)
		values('$post_id','$id','$comment','$user_com_name',NOW())";
		$run=mysqli_query($con,$insert);
...
$get_posts= "select * from posts where id='$u_id' ORDER by 1 DESC LIMIT 10";
		$run_posts = mysqli_query($con,$get_posts);
		while ($row_posts = mysqli_fetch_array($run_posts)){
           ...
           $getuser= "select user_email from users where id='$u_id' ";
           ...
           $row = mysqli_fetch_array($run_user);
           	...
           	if($u_email !=$user_email){
           		echo "<script>window.open('my_post.php?u_id=$id', '_self')</script>";
         ...
         echo"
          <div class='row'>
             ...
             <div class='row' id ='find_people'>
                 ...
                 <div class='col-sm-6'>
                 <a style='text-decoration:none;cursor:pointer;color:#337AFF;'
                 href= 'user_profile.php?u_id=$id'>
